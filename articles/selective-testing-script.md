...
上記の処理により、変更されたファイルに影響するターゲットが特定できたため、テストターゲットも特定していきます。

4でマッピングした結果を利用することで、変更されたターゲットに依存するテストターゲットを特定できます。

:::message alert
5の処理にて空配列が返却された場合、XCTestPlanの実行対象がなくなってしまう。  
...
※ 念のため、毎日朝と夕方に全てのテストを実行することで差分テストにした場合の安全性も担保しています。

1ヶ月ほど運用した結果、実測値でも **50%** ほどCIの実行時間が削減されており、かなり大きな成果を得ることができました。
![](/images/selective-testing-script/asc-xcodecloud-selective-testing-result.png)

今回、ClineやChatGPT等のAIをフルに活用して開発したところ1日程度で上記のスクリプトは完成しました！どちらかというとXcodeCloud側の仕様調査やAppleのドキュメントを読んでいる時間のほうが長かったように思います。  
Appleのドキュメントに関してはDeepResearchを利用したりもしましたが、知りたい部分が書かれていないこともあり（XcodeCloud内のGit周りなど）調査に手こずった印象です。
